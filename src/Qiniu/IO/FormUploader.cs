...
namespace Qiniu.IO
{
    ...
    public class FormUploader
    {
        ...
        /// <param name="token">上传凭证</param>
        /// <returns>上传文件后的返回结果</returns>
        public async Task<HttpResult> UploadFileAsync(string file, string saveKey, string token)
        {
            HttpResult result = new HttpResult();
        ...
        /// <param name="extraParams">用户自定义的附加参数</param>
        /// <returns>上传文件后的返回结果</returns>
        public async Task<HttpResult> UploadFileAsync(string file, string saveKey, string token, Dictionary<string, string> extraParams)
        {
            HttpResult result = new HttpResult();
        ...
        public async Task<HttpResult> UploadStreamAsync(Stream stream, string saveKey, string token)
        {
            ...
            {
                StringBuilder sb = new StringBuilder();
                sb.AppendFormat("[{0}] [FormUpload] Error: \n", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.ffff"));
                Exception e = ex;
                while (e != null)
                {
                    sb.Append(e.Message + " ");
