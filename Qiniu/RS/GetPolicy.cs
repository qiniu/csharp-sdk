...
namespace Qiniu.RS
{
    ...
    public class GetPolicy
    {
        ...
        public static string MakeRequest(string baseUrl, string attname, UInt32 expires = 3600, Mac mac = null)
        {
            baseUrl += ("?attname=" + System.Web.HttpUtility.UrlEncode(attname));
            if (mac == null)
            {
                mac = new Mac(Config.ACCESS_KEY, Config.Encoding.GetBytes(Config.SECRET_KEY));
            }
        ...
        public static string MakeBaseUrl(string domain, string key)
        {
            ...
            return string.Format("{0}/{1}", domain, key);
        }
    }
}
